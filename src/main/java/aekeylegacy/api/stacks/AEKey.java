...
import aekeylegacy.api.config.FuzzyMode;
 ...
 * {@link net.minecraft.nbt.NBTTagCompound}. To account for common indexing scenarios, a key is (optionally) split into a
...
public abstract class AEKey {
    ...
    /**
     * Same as {@link #toTag()}, but includes type information so that {@link #fromTagGeneric(NBTTagCompound)} can restore
     * this particular type of key withot knowing the actual type beforehand.
     */
    public final NBTTagCompound toTagGeneric() {
        var tag = toTag();
        tag.setString("#c", getType().getId().toString());
        return tag;
    }
     ...
     * @param world  World where the stacks were being held
